...
router.get("/:id", async (req, res) => {
  ...
  try {
    const [rows] = await req.mysql.query(
      "SELECT * FROM addtocart WHERE id = ?",
      [userId]
    );
...
router.post("/", async (req, res) => {
  ...
  try {
    const result = await req.mysql.query(
      "INSERT INTO addtocart (username, email) VALUES (?, ?)",
      [username, email]
    );
...
router.put("/:id", async (req, res) => {
  ...
  try {
    const result = await req.mysql.query(
      "UPDATE addtocart SET username = ?, email = ? WHERE id = ?",
...
router.delete("/:id", async (req, res) => {
  ...
  try {
    const result = await req.mysql.query("DELETE FROM addtocart WHERE id = ?", [
      userId,
    ]);
