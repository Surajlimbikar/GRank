...
router.get("/", (req, res) => {
  const query = "SELECT * FROM 	microsoft";
...
router.get("/:id", async (req, res) => {
  ...
  try {
    const [rows] = await req.mysql.query(
      "SELECT * FROM microsoft WHERE id = ?",
      [userId]
    );
...
router.post("/", async (req, res) => {
  ...
  try {
    const result = await req.mysql.query(
      "INSERT INTO microsoft (username, email) VALUES (?, ?)",
      [username, email]
    );
...
router.put("/:id", async (req, res) => {
  ...
  try {
    const result = await req.mysql.query(
      "UPDATE microsoft SET username = ?, email = ? WHERE id = ?",
...
router.delete("/:id", async (req, res) => {
  ...
  try {
    const result = await req.mysql.query("DELETE FROM microsoft WHERE id = ?", [
      userId,
    ]);
