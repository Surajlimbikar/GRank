...
router.get("/", (req, res) => {
  const query = "SELECT * FROM 	rediffmail";
...
router.get("/:id", async (req, res) => {
  ...
  try {
    const [rows] = await req.mysql.query(
      "SELECT * FROM rediffmail WHERE id = ?",
      [userId]
    );
...
router.post("/", async (req, res) => {
  ...
  try {
    const result = await req.mysql.query(
      "INSERT INTO rediffmail (username, email) VALUES (?, ?)",
      [username, email]
    );
...
router.put("/:id", async (req, res) => {
  ...
  try {
    const result = await req.mysql.query(
      "UPDATE rediffmail SET username = ?, email = ? WHERE id = ?",
...
router.delete("/:id", async (req, res) => {
  ...
  try {
    const result = await req.mysql.query(
      "DELETE FROM rediffmail WHERE id = ?",
      [userId]
    );
