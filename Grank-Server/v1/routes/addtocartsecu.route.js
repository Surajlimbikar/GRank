...
router.get("/", (req, res) => {
  const query = "SELECT * FROM addtocartsecu";
...
router.get("/:id", async (req, res) => {
  ...
  try {
    const [rows] = await req.mysql.query(
      "SELECT * FROM addtocartsecu WHERE id = ?",
      [userId]
    );
...
router.post("/", async (req, res) => {
  ...
  try {
    const result = await req.mysql.query(
      "INSERT INTO addtocartsecu (username, email) VALUES (?, ?)",
      [username, email]
    );
...
router.put("/:id", async (req, res) => {
  ...
  try {
    const result = await req.mysql.query(
      "UPDATE addtocartsecu SET username = ?, email = ? WHERE id = ?",
...
router.delete("/:id", async (req, res) => {
  ...
  try {
    const result = await req.mysql.query(
      "DELETE FROM addtocartsecu WHERE id = ?",
      [userId]
    );
