...
let initiate_payment = function (data, config, res) {
  console.log("initial-data", data); //f

  function isInteger(amt) {
    var regexp = /^\d+$/;
  ...
  function checkArgumentValidation(data, config) {
    console.log("checkArgument", data); //f
    if (!data.name.trim()) {
      res.json({
        status: 0,
    ...
    if (!data.amount.trim() || !isInteger(data.amount)) {
      res.json({
        status: 0,
        data: "Mandatory Parameter amount can not empty and must be in decimal ",
      });
    }
    if (!data.txnid.trim() || !isInteger(data.txnid)) {
      res.json({
        status: 0,
    ...
    if (
      !data.email.trim() ||
      (!isValidEmail(data.email.trim()) && !isInteger(data.email.trim()))
    ) {
      res.json({
        status: 0,
  ...
  function form() {
    ...
    console.log("data.productinfo", data.productinfo); //f
    return form;
  }
  ...
  util.call(call_url, form()).then(function (response) {
    console.log("hash-reponsedata", response.data);
  ...
  function generateHash() {
    ...
    console.log("generatehash-data", data.productinfo); //f
    return data.hash;
  }
};
